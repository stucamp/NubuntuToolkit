...
if [[ ! -L "/usr/local/bin/nub" ]]; then
    ...
    sudo ln -s -T "$SCRIPTPATH/setup.sh" /usr/local/bin/nub
    echo 'You can use the Nubuntu-ToolKit by typing the following command from anywhere:'
    echo -e "${GREEN}sudo nub$ENDCOLOR"
    echo
fi
