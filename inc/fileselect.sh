...
if [ ${#FILELIST[@]} -eq 0 ]; then
    ...
    FILECHOICE=$(whiptail --title "Ubuntu ToolKit - Restore Backup" --menu "Select the backup file you want to restore:" --backtitle "$BACKTITLE" --fb --ok-button "Select" $LINES $COLUMNS $NETLINES "${FILELIST[@]}" 3>&1 1>&2 2>&3)
fi
